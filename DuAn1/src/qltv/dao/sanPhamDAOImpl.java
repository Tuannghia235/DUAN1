...
public class sanPhamDAOImpl implements sanPhamDAO {
    ...
    public int deleteSanPham(int id) {
        ...
        try {
            ...
            cons.setAutoCommit(false); // Tắt chế độ auto-commit để thực hiện giao dịch
            ...
            cons.commit(); // Commit giao dịch nếu tất cả các lệnh DELETE đều thành công
            return rowsAffected1 + rowsAffected2 + rowsAffected3;
        } catch (Exception e) {
            e.printStackTrace();
            if (cons != null) {
                try {
                    cons.rollback(); // Rollback giao dịch nếu có lỗi xảy ra
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            }
